    ...
    expenses
      ...
      .forEach((data: ExpenseByCategorySummary) => {
        if (!filtered[data.category]) {
          filtered[data.category] = { name: data.category, amount: 0, color: getColor(data.category, idx++) };
        }
              ...
              <div className="space-y-2">
                {aggregatedData.map((item, index) => (
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-700">{formatRupiah(item.amount)}</span>
                  </div>
                ))}
                <div className="flex justify-between pt-2 border-t border-gray-100">
                  <span className="text-xs font-medium text-gray-500">Total</span>
                  <span className="text-xs font-bold text-gray-800">{formatRupiah(totalAmount)}</span>
                </div>
              </div>
            </div>
          )}
