...
const NewCategoryPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to create category. The name may already exist.");
  ...
  return (
    <div className="pb-10 w-full">
      <Breadcrumb items={[{ label: "Categories", href: "/categories" }, { label: "Add New Category" }]} />
                ...
                {PRESET_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl transition-all hover:scale-110 ${
                      form.color === c ? "ring-2 ring-offset-2 ring-gray-500 scale-110" : ""
                    }`}
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 w-fit">
                ...
                <label className="w-7 h-7 rounded-lg overflow-hidden border border-gray-200 cursor-pointer hover:border-gray-400 transition-colors shrink-0">
                  <input
                    ...
                    className="w-9 h-9 -ml-1 -mt-1 cursor-pointer border-0"
                  />
                </label>
