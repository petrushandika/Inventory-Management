...
import { useGetProductsQuery, useGetCategoriesQuery, useUpdateProductMutation, NewProduct } from "@/state/api";
...
const EditProductPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Partial<NewProduct> & { productId: string } = {
        productId,
        name: form.name.trim(),
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Wireless Headphones" className={inputCls} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              ...
              <div>
                ...
                <input type="number" min="0" max="5" step="0.1" value={form.rating} onChange={(e) => setForm((f) => ({ ...f, rating: e.target.value }))} placeholder="e.g. 4.5" className={inputCls} />
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-4 bg-gray-50 rounded-b-2xl border-t border-gray-100 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <Link href="/products" className="px-5 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 hover:bg-gray-100 rounded-lg transition-colors">Cancel</Link>
