...
import { useGetSuppliersQuery, useUpdateSupplierMutation } from "@/state/api";
...
const EditSupplierPage = () => {
  ...
  return (
    <div className="pb-10 w-full">
      <Breadcrumb items={[{ label: "Suppliers", href: "/suppliers" }, { label: "Edit Supplier" }]} />

      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">Edit Supplier</h1>
        <p className="text-sm text-gray-500 mt-1">Update the supplier&apos;s information.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Supplier Name <span className="text-red-400">*</span></label>
              <input required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. TechSource Global" className={inputCls} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input type="email" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} placeholder="contact@supplier.com" className={inputCls} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Phone Number</label>
                <input type="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} placeholder="+62-21-000-0000" className={inputCls} />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-3">Location / Address</label>
              <LocationPicker value={form.address} onChange={(address) => setForm((f) => ({ ...f, address }))} />
            </div>
          </div>
          ...
          <div className="px-6 py-4 bg-gray-50 rounded-b-2xl border-t border-gray-100 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <Link href="/suppliers" className="px-5 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 hover:bg-gray-100 rounded-lg transition-colors">Cancel</Link>
